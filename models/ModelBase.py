...
class ModelBase(object):
    def __init__(self, is_training=False,
        ...
        self.target_iter = 10 #self.options.get('target_iter',0)
    ...
    def ask_write_preview_history(self, default_value=False):
        ...
        self.options['write_preview_history'] = True #io.input_bool(f"Write preview history", default_write_preview_history, help_message="Preview history will be writed to <ModelName>_history folder.")

        if self.options['write_preview_history']:
            if io.is_support_windows():
                self.choose_preview_history = False # io.input_bool("Choose image for the preview history", False)
            elif io.is_colab():
                self.choose_preview_history = False #io.input_bool("Randomly choose new image for preview history", False, help_message="Preview image history will stay stuck with old faces if you reuse the same model on different celebs. Choose no unless you are changing src/dst to a new person")

    def ask_target_iter(self, default_value=0):
        self.options['target_iter'] = self.load_or_def_option('target_iter', default_value)
        # self.options['target_iter'] = default_target_iter

    def ask_random_flip(self):
        self.options['random_flip'] = self.load_or_def_option('random_flip', True)
    ...
    def ask_batch_size(self, suggest_batch_size=None, range=None):
        batch_size = self.load_or_def_option('batch_size', suggest_batch_size or self.batch_size)
