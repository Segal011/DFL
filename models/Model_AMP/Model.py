...
class AMPModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            resolution = 128 #io.input_int("Resolution", default_resolution, add_info="64-640", help_message="More resolution requires more VRAM and time to train. Value will be adjusted to multiple of 32 .")
            resolution = np.clip ( (resolution // 32) * 32, 64, 640)
            self.options['resolution'] = resolution
            self.options['face_type'] = 'wf' #io.input_str ("Face type", default_face_type, ['f','wf','head'], help_message="whole face / head").lower()
        ...
        if self.is_first_run():
            ...
            self.options['inter_dims'] = np.clip (default_inter_dims, 32, 2048 )

            e_dims = np.clip ( default_e_dims,
                16, 256 )
            self.options['e_dims'] = e_dims + e_dims % 2

            d_dims = np.clip ( default_d_dims,
                16, 256 )
            ...
            morph_factor = np.clip ( default_morph_factor, 0.1, 0.5 )
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['gan_power'] = default_gan_power#np.clip ( io.input_number ("GAN power", default_gan_power, add_info="0.0 .. 5.0", help_message="Forces the neural network to learn small details of the face. Enable it only when the face is trained enough with random_warp(off), and don't disable. The higher the value, the higher the chances of artifacts. Typical fine value is 0.1"), 0.0, 5.0 )

            if self.options['gan_power'] != 0.0:
                gan_patch_size = np.clip (
                    default_gan_patch_size, # io.input_int("GAN patch size", default_gan_patch_size, add_info="3-640", help_message="The higher patch size, the higher the quality, the more VRAM is required. You can get sharper edges even at the lowest setting. Typical fine value is resolution / 8." ),
                    3, 640 )
                self.options['gan_patch_size'] = gan_patch_size

                gan_dims = np.clip (
                    default_gan_dims, #io.input_int("GAN dimensions", default_gan_dims, add_info="4-512", help_message="The dimensions of the GAN network. The higher dimensions, the more VRAM is required. You can get sharper edges even at the lowest setting. Typical fine value is 16." ),
                    4, 512 )
                self.options['gan_dims'] = gan_dims

            self.options['ct_mode'] = default_ct_mode #io.input_str (f"Color transfer for src faceset", default_ct_mode, ['none','rct','lct','mkl','idt','sot'], help_message="Change color distribution of src samples close to dst samples. If src faceset is deverse enough, then lct mode is fine in most cases.")
    ...
    def get_MergerConfig(self):
        morph_factor = np.clip ( 1.0, 0.0, 1.0 )
